...
def run_export(consumer_id, export_type, job_id):
    logging.info({"event":"export_started","jobId":job_id,"consumerId":consumer_id,"type":export_type})

    conn = get_db()
    cur = conn.cursor()

    cur.execute("SELECT last_exported_at FROM watermarks WHERE consumer_id=%s", (consumer_id,))
    row = cur.fetchone()
    ...
    logging.info({"event":"export_completed","jobId":job_id,"rows":len(rows)})